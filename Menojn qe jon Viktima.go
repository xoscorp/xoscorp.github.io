...
func (s *Scanner) scanSSRF() {
        ...
        ssrfPayloads := []string{
                ...
                "http://d402c29e23d66ef2d994gt7tibhyyyyyn.oast.me",
                "https://d402c29e23d66ef2d994gt7tibhyyyyyn.oast.me",
        }
        ...
        // Create SSRF log file
        ssrfLog, _ := os.Create("ssrf_tests.log")
        defer ssrfLog.Close()

        // Log scan start time
        scanStart := time.Now()
        ssrfLog.WriteString(fmt.Sprintf("=== SSRF SCAN STARTED AT: %s ===\n\n", scanStart.Format("2006-01-02 15:04:05.000 MST")))

        fmt.Printf("%s[*] Testing %d parameters for SSRF (logging to ssrf_tests.log)...%s\n", ColorCyan, maxTest, ColorReset)
        fmt.Printf("%s[*] Scan started at: %s%s\n", ColorCyan, scanStart.Format("15:04:05.000"), ColorReset)
        ...
        for i := 0; i < maxTest; i++ {
                ...
                for _, payload := range ssrfPayloads {
                        ...
                        // Record exact time before request
                        testStart := time.Now()
                        timestamp := testStart.Format("2006-01-02 15:04:05.000 MST")

                        // Log to file with detailed info
                        logEntry := fmt.Sprintf("[%s] Testing SSRF\n", timestamp)
                        logEntry += fmt.Sprintf("  └─ Base URL: %s\n", param.URL)
                        logEntry += fmt.Sprintf("  └─ Parameter: %s\n", param.ParamName)
                        logEntry += fmt.Sprintf("  └─ Payload: %s\n", payload)
                        logEntry += fmt.Sprintf("  └─ Full Test URL: %s\n", testURL)
                        logEntry += fmt.Sprintf("  └─ Unix Timestamp: %d\n", testStart.Unix())
                        logEntry += fmt.Sprintf("  └─ Milliseconds: %d\n\n", testStart.UnixMilli())
                        ssrfLog.WriteString(logEntry)

                        // Print to console
                        fmt.Printf("%s[%s] Testing param '%s' on %s%s\n",
                                ColorCyan, testStart.Format("15:04:05.000"), param.ParamName, param.URL, ColorReset)
                        ...
                        if err == nil {
                                ...
                                ssrfLog.WriteString(fmt.Sprintf("  ✓ Response received in %v | Status: %d\n\n", duration, resp.StatusCode))
                                ...
                                if strings.Contains(string(body), "ami-id") ||
                                        ...
                                        vuln := Vulnerability{
                                                ...
                                                URL:         param.URL,
                                                Parameter:   param.ParamName,
                                                Evidence:    fmt.Sprintf("SSRF to %s succeeded at %s", payload, timestamp),
                                                Tool:        "custom",
                                                Timestamp:   testStart,
                                        ...
                                        criticalLog := fmt.Sprintf("*** SSRF CONFIRMED ***\n")
                                        criticalLog += fmt.Sprintf("  [!] Time: %s\n", timestamp)
                                        criticalLog += fmt.Sprintf("  [!] URL: %s\n", param.URL)
                                        criticalLog += fmt.Sprintf("  [!] Parameter: %s\n", param.ParamName)
                                        criticalLog += fmt.Sprintf("  [!] Payload: %s\n\n", payload)
                                        ssrfLog.WriteString(criticalLog)
                                        break
                                }
                        } else {
                                ssrfLog.WriteString(fmt.Sprintf("  ✗ Request failed: %v\n\n", err))
                        }
                        tested++
                }
        }
        ...
        scanEnd := time.Now()
        totalDuration := scanEnd.Sub(scanStart)

        ssrfLog.WriteString(fmt.Sprintf("\n=== SSRF SCAN COMPLETED AT: %s ===\n", scanEnd.Format("2006-01-02 15:04:05.000 MST")))
        ssrfLog.WriteString(fmt.Sprintf("Total Duration: %v\n", totalDuration))
        ssrfLog.WriteString(fmt.Sprintf("Total Tests: %d\n", tested))

        fmt.Printf("%s[✓] SSRF: Tested %d parameter combinations in %v%s\n", ColorGreen, tested, totalDuration, ColorReset)
        fmt.Printf("%s[*] Scan completed at: %s%s\n", ColorGreen, scanEnd.Format("15:04:05.000"), ColorReset)
        fmt.Printf("%s[*] Check http://8f2487e5b6b6958eac86gthebphyyyyyb.oast.pro for callbacks%s\n", ColorYellow, ColorReset)
        fmt.Printf("%s[*] All tests with timestamps logged to: ssrf_tests.log%s\n", ColorGreen, ColorReset)
}
...
func (s *Scanner) scanBlindXSS() {
        ...
        blindXSSPayloads := []string{
                ...
                if s.Result.Parameters == nil || len(s.Result.Parameters) == 0 {
                fmt.Printf("%s[!] No parameters found to test%s\n", ColorYellow, ColorReset)
                return
        }
...
func (s *Scanner) generateReport() {
        ...
        os.WriteFile(s.Output, reportJSON, 0644)
